...
- `Provider lock` workflow — monthly (and on demand) moves the zone locks to the newest provider the constraint allows, validates the zones, and opens a pull request. Dependabot cannot do this: it only rewrites a lock alongside a manifest change, and the manifests here are generated by Terragrunt and never committed
