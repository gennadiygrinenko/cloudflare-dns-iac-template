...
Keeping the lock current is split between two mechanisms, because neither covers both halves. Dependabot owns the constraint in `versions.tf` and will propose `~> 6.0` when a major appears; it will not move the lock from 5.23.0 to 5.24.0, since it only rewrites a lock as a side effect of changing a manifest, and nothing in the manifest changes. The scheduled `Provider lock` workflow owns that half: it runs `init -upgrade` plus `providers lock` per zone and opens a pull request when the resolved build moves. It also validates the zones itself, because pull requests opened with `GITHUB_TOKEN` do not trigger workflows and an unchecked lock bump is exactly the change you want checked.
